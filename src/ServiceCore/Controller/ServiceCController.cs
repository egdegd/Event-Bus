...
namespace WebAPI.Core.Controller
{
    ...
    public class ServiceCController : ApiController
    {
        ...
        {
            var client = new HttpClient();
            var response = client.GetAsync("http://localhost:9000/api/eventbus/sendmsg?name=serviceC").Result;
            var result = response.Content.ReadAsStringAsync().Result;
            if (result != "\"no new messages\"")
            {
                Message actualResultFromGet = JsonConvert.DeserializeObject<Message>(result);
                Console.WriteLine($"serviceC received message \"{actualResultFromGet.Text}\" from {actualResultFromGet.From}");
            }
        ...
        {
            var client = new HttpClient();
            var response = client.GetAsync("http://localhost:9000/api/eventbus/sendevent?name=serviceC").Result;
            var result = response.Content.ReadAsStringAsync().Result;
            if (result != "\"no new events\"")
            {
                Event actualResultFromGet = JsonConvert.DeserializeObject<Event>(result);
                Console.WriteLine($"serviceC received an event notification from {actualResultFromGet.Organizer} with description \"{actualResultFromGet.Description}\"");
            }
            return Request.CreateResponse(HttpStatusCode.OK, "OK!", new MediaTypeHeaderValue("text/json"));
        }
